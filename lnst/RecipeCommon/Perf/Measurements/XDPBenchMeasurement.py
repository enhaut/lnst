from lnst.RecipeCommon.Perf.Measurements.Results.AggregatedXDPBenchMeasurementResults import AggregatedXDPBenchMeasurementResults
...
class XDPBenchMeasurement(BaseFlowMeasurement):
    ...
    def collect_results(self):
        ...
        for test_flow in test_flows:
            ...
            flow_results.generator_results = self._parse_generator_results(
                ...
                test_flow.flow.duration + test_flow.flow.warmup_duration * 2,
            )
    ...
    def _parse_generator_results(self, job: Job, duration: int):
        results = ParallelPerfResult()  # container for multiple instances of pktgen

        for device, raw_result in job.result.items():
            result = SequentialPerfResult()  # instance (device) of pktgen

            cumulative_sample = PerfInterval(
                raw_result["result"]["packets"],
                raw_result["result"]["duration"],
                "packets",
                raw_result["current"]["started"],
            )
            result.extend(cumulative_sample.resample(duration))
            results.append(result)

        return results

    def _parse_receiver_results(self, job: Job):
        result = ParallelPerfResult()  # just a placeholder to keep data structure same as other Measurements
        results = SequentialPerfResult()  # single instance of xdp-bench

        for i, sample in enumerate(job.result):
            results.append(PerfInterval(sample["rx"], 1, "packets", i))
    ...
    def _aggregate_flows(self, old_flow, new_flow):
        ...
        new_result = AggregatedXDPBenchMeasurementResults(measurement=self, flow=new_flow.flow)
    ...
    def _report_flow_results(cls, recipe, flow_results):
        ...
        desc.append("Generator generated: {tput:,f} {unit} per second."
                .format(tput=generator.average,
                        unit=generator.unit))
        desc.append("Receiver processed: {tput:,f} {unit} per second."
