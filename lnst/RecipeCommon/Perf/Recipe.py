...
class RecipeResults(object):
    ...
    """
        ...
        That results to slice measurement just for "interesting" part not including 
    ...
    def _get_real_flow_times(self, flows: List[BaseMeasurementResults]):
        ...
        if isinstance(flows[-1], FlowMeasurementResults):
            logging.info(f"Results alignment: Using times of flow measurement: {flows[-1]}")
            return flows[-1].warmup_end, flows[-1].warmdown_start
        else:
            logging.info("Results alignment: Using times from latest start and earliest end")
    ...
    def time_aligned_results(self) -> "RecipeResults":
        ...
        for i in range(self.recipe_conf.iterations):
            ...
            real_times = self._get_real_flow_times(iteration_results_group)
